...
def onUidFound(bytearray):
    ...
    return {'type':'uid', 'content':{'namespace':namespace, 'instance':instance}}
...
def onPacketFound(packet):
    ...
    if len(data) >= 20 and data[19] == 0xaa and data[20] == 0xfe:
        ...
        if frameType == Eddystone.url.value:
            result = onUrlFound(decodeUrl(data[27:22 + serviceDataLength]))
        elif frameType == Eddystone.uid.value:
            result = onUidFound(data[27:22 + serviceDataLength])
